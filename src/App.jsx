      ...
      <div className="app">
        ...
        {!keySaved && (
          <div className="input-section">
            <label className="question-label">API Key de Google Gemini</label>
            <input
              type="password"
              className="question-input"
              style={{ fontSize: 15, fontStyle: "normal" }}
              placeholder="Pega aquí tu API Key de aistudio.google.com"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && apiKey.trim() && setKeySaved(true)}
            />
            <button
            ...
            <p className="key-hint">Tu clave nunca se guarda ni se comparte</p>
          </div>
        )}
